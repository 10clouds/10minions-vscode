...
const getLanguegeFileExtension = () => config.language === "typescript" ? "ts" : "js";
...
const createScoreTestFiles = async (testData: TestRequiredData) => {
    ...
    if (withSelectedText) {
        createTestFile(selectedText, `${testFileNamePrefix}selectedText.txt`, TestType.SCORE);
    }
    createTestFile(originalContent, `${testFileNamePrefix}original.txt`, TestType.SCORE);
    createTestFile(userQuery, `${testFileNamePrefix}userQuery.txt`, TestType.SCORE);
    createTestFile(SCORE_TEST_INITIAL_CONTENT, `${testFileNamePrefix}tests.json`, TestType.SCORE);
};


const createProcedureTestFiles = async (testData: TestRequiredData, testType: TestType) => {
    const { finalContent, originalContent, userQuery, modificationDescription, modificationProcedure } = testData;
    const testDirPath = createTestsDirectory(testType);
    const testFileNamePrefix = `${testDirPath}/`;

    if (testType === TestType.CREATE_PROCEDURE) {
        createTestFile(modificationDescription, `${testFileNamePrefix}modification.txt`, testType);
    }

    createTestFile(originalContent, `${testFileNamePrefix}original.txt`, testType);
    createTestFile(modificationProcedure, `${testFileNamePrefix}procedure.txt`, testType);
...
const prepareTestFiles = async () => {
    ...
    try {
        const minionTaskSnapshot = await admin.firestore().collection('minionTasks').where('id', '==', "001279e4-8635-4d25-b9cd-8c3937693581").get();

        let testData: TestRequiredData = {} as TestRequiredData;
    
        minionTaskSnapshot.forEach((doc) => {
            const { selectedText, finalContent, originalContent, userQuery, modificationDescription, modificationProcedure } = doc.data();
            testData = { selectedText, finalContent, originalContent, userQuery, modificationDescription, modificationProcedure };
        });

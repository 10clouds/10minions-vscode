...
import { LOG_PLAIN_COMMENT_MARKER as LOG_FALLBACK_COMMENT_MARKER, LOG_NO_FALLBACK_MARKER as LOG_NORMAL_MODIFICATION_MARKER, applyMinionTask } from "../strategies/utils/applyMinionTask";
...
async function runTest({ fileName, iterations = defaultIternationsNumber }: { fileName: string; iterations?: number }) {
  ...
  for (let i = 0; i < iterations; i++) { 
    ...
    statistics.total++;
    let includesNormalModification = execution.logContent.includes(LOG_NORMAL_MODIFICATION_MARKER);
    let includesFallbackComment = execution.logContent.includes(LOG_FALLBACK_COMMENT_MARKER);

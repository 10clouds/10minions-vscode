...
export class VSCommandHistoryManager implements CommandHistoryManager {
  ...
  constructor(private context: vscode.ExtensionContext) {
    ...
    let commandHistory = this.context.globalState.get(
      '10minions.commandHistory',
    );
    ...
    this.commandHistory = commandHistory as {
      command: string;
      timeStamp: number;
    }[];
  ...
  async updateCommandHistory(prompt: string) {
    ...
    this.commandHistory = this.commandHistory.filter(
      (commandObj) => commandObj.command.toLowerCase() !== prompt.toLowerCase(),
    );
  ...
  private getRelatedPreviousCommands(input: string): string[] {
    ...
    const historyToSearchCommandHistory = [...this.commandHistory];
    const historyToSearchBaseCommands = [...BASE_COMMANDS];

    // Step 2: Calculate relevance scores for each command in the history
    let scoredItemsCommandHistory: { command: string; score: number }[] =
      historyToSearchCommandHistory.map((commandObj) => {
        const command = commandObj.command;
        const score =
          command.toLowerCase().includes(input.toLowerCase()) &&
          command.toLowerCase() !== input.toLowerCase()
            ? 1
            : 0;
        return { command, score };
      });
    let scoredItemsBaseCommands: { command: string; score: number }[] =
      historyToSearchBaseCommands.map((commandObj) => {
        const command = commandObj.command;
        const score =
          command.toLowerCase().includes(input.toLowerCase()) &&
          command.toLowerCase() !== input.toLowerCase()
    ...
    scoredItemsCommandHistory = scoredItemsCommandHistory.filter(
      (item) =>
        item.score > 0 &&
        item.command.length <= 400 &&
        !item.command.includes('\n'),
    );
    scoredItemsBaseCommands = scoredItemsBaseCommands.filter(
      (item) =>
        item.score > 0 &&
        item.command.length <= 400 &&
        !item.command.includes('\n'),
    ...
    return [...scoredItemsCommandHistory, ...scoredItemsBaseCommands].map(
      (item) => item.command,
    );
  }

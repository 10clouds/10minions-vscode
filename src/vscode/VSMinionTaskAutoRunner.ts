...
class TodoCodeLensProvider implements vscode.CodeLensProvider {
  public provideCodeLenses(document: vscode.TextDocument): vscode.CodeLens[] {
    ...
    for (let lineNum = 0; lineNum < document.lineCount; lineNum++) {
      ...
      if (match) {
        const rangeForSelection = new vscode.Range(
          new vscode.Position(lineNum, match.index),
          new vscode.Position(lineNum, match.index + match[0].length),
        );
        const selection = new vscode.Selection(
          rangeForSelection.start,
          rangeForSelection.end,
        );

        const existing = getMinionTasksManager().getExecutionByUserQueryAndDoc(
          todoMessage,
          document,
        );
        if (existing) {
          const todoCodeLens = new vscode.CodeLens(rangeForSelection, {
            command: '10minions.stopTask',
            title: existing.executionStage,
          ...
          const todoCodeLens = new vscode.CodeLens(rangeForSelection, {
            command: '10minions.executeTask',
            title: 'Execute task',
            arguments: [document.uri, selection, todoMessage],
          });
...
export class MinionTaskAutoRunner {
  ...
  scanTODOComments(rootDir: string) {
    ...
    tsFiles.forEach((file: string) => {
      const lines = fs
        .readFileSync(path.join(rootDir, file))
        .toString()
        .split('\n');
      ...
      lines.forEach((line: string, lineNumber: number) => {
        if (line.includes('//TODO:')) {
          ...
          this.diagnosticsCollection.set(
            vscode.Uri.file(path.join(rootDir, file)),
            [diagnostic],
          ); // use this.diagnosticsCollection instead of undefined 'diagnostics'
        }
      });
    });
  }
  ...
  constructor(context: vscode.ExtensionContext) {
    this.diagnosticsCollection =
      vscode.languages.createDiagnosticCollection('todoDiagnostics'); // initialization of diagnosticsCollection

    this.monitorFilesForTasks();

    context.subscriptions.push(
      vscode.commands.registerCommand(
        '10minions.executeTask',
        async (
          uri: vscode.Uri,
  ...
  private checkForTaskComments(document: vscode.TextDocument) {
    const taskCommentKeyword = vscode.workspace
      .getConfiguration('10minions')
      .get<string>('taskCommentKeyword');

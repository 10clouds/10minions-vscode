  ...
  ({ execution, ...props }: { execution: ExecutionInfo } & React.HTMLAttributes<HTMLDivElement>, ref: React.ForwardedRef<HTMLDivElement>) => {
    ...
    return (
      <div
        ...
        key={execution.id}
        className={`execution p-4 mb-4 rounded flex flex-col ${className}`}
      ...
      >
        <div className="flex justify-between">
          ...
          {isExpanded ? (
            <ChevronDownIcon title="Contract" onClick={() => setIsExpanded(false)} className="h-6 w-6 cursor-pointer" />
          ) : (
            <ChevronUpIcon title="Expand" onClick={() => setIsExpanded(true)} className="h-6 w-6 cursor-pointer" />
          )}
          <XMarkIcon
        ...
        {isExpanded && (
          <>
            <div className="mb-2 mt-2" onClick={() => setIsInputOpen(true)}>
              {isInputOpen ? (
                <textarea
                  style={{
            ...
            <div>
              ...
              <div className="flex justify-between mb-2 items-center">
                <span>
                <RobotIcon className={`w-4 h-4 inline-flex ${!execution.stopped ? "motion-safe:animate-bounce" : ""}`} />
                  {' '}

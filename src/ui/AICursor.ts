...
export async function startWritingComment(aiCursor: AICursor) {
  ...
  let adjustedPosition = new vscode.Position(aiCursor.position.line, 0);
  ...
  await editDocument(async (edit) => {
    ...
    edit.insert(
      aiCursor.document.uri,
      adjustedPosition,
      getCommentForLanguage(aiCursor.document.languageId, '')
    );
  });

  aiCursor.position = adjustedPosition.translate(1, 0);
}